                  ...
                  <>
                    ...
                    {(['ADMIN', 'MODERATOR', 'EXPERT'].includes(role) || Number((JSON.parse(localStorage.getItem('afrikoba_user') || '{}').id || 0)) === Number(statement.project.owner_user_id)) && (
                      <>
                        ...
                        <DocBtn label={t('projects.evidence_pack_pdf')} chip="PDF" onClick={() => downloadDocumentPdf(`/api/projects/${statement.project.id}/evidence-pack/pdf`, `project-${statement.project.id}-evidence-pack.pdf`)} />
                        <DocBtn label={t('projects.archive_zip')} chip="ZIP" onClick={() => downloadFinanceCsv(`/api/projects/${statement.project.id}/archive`, `afrikoba-project-${statement.project.id}-archive.zip`)} />
        ...
        <div className="card">
          ...
          {performance && (
            <div style={{ marginBottom: 16 }}>
              ...
              <button className="btn btn-secondary" style={{ padding: '4px 10px', fontSize: 12, marginTop: 8 }} onClick={() => downloadDocumentPdf('/api/projects/mine/performance/pdf', 'my-project-portfolio.pdf')}>{t('projects.portfolio_pdf')}</button>
            </div>
          )}
      ...
      <div className="card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          ...
          <div className="doc-grid">
            ...
            {OPS_ROLES.includes(role) && <DocBtn label={t('projects.kpi_csv')} chip="CSV" onClick={() => downloadFinanceCsv('/api/projects/ops/kpi/export', 'platform-kpi.csv')} />}
            {OPS_ROLES.includes(role) && <DocBtn label={t('projects.kpi_pdf')} chip="PDF" onClick={() => downloadDocumentPdf('/api/projects/ops/kpi/pdf', 'platform-kpi.pdf')} />}
          </div>
        </div>
        {opsBook ? (
          <>
