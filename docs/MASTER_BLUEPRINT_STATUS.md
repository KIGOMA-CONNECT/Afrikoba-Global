...
- 🔶 Merchant, cross-border, commerce = partial (cross-border remittance only via country carrier routes; merchant platform: `merchantService` + QR + invoices + payroll + procurement shipped; insurance shipped; cards shipped: Luhn-valid PAN/CVV-once issuance, sha256 hashing, limits, freeze/block, holds via engine (CUSTOMER_WALLET/CARD_HOLD + decline reasons), admin settle/refund idempotent on auth_reference, statements/summary — `scripts/test-cards.js` (71 checks) + `scripts/test-insurance.js` (37 checks) + `scripts/test-qr.js` (33 checks), suites 44/43/42).
- ✅ **SACCOS Digital Core — Foundation (increment 1)**: migration 101 `saccos` (config-driven JSONB setup, unique code/name) + `saccos_compliance` boundary (regulatory_status TECH_INFRA) + `saccos_members` (per-SACCOS member_number, INVITED→ACTIVE/SUSPENDED/EXITED, re-invite on exit). Org registration, invite-by-identity-phone, accept, suspend, exit; OWNER/BOARD/MEMBER RBAC; cross-entity isolation (non-member 404) + platform-ADMIN oversight. Routes env-gated (`SACCOS_ENABLED=false` default; CI=true). `scripts/test-saccos-foundation.js` (39 checks, suite 45).
- ✅ **SACCOS Digital Core — Shares (increment 2)**: migration 102 share subscription ledgered on the shared double-entry core (`debitWallet` DR CUSTOMER_WALLET / CR per-entity EQUITY `SACCOS<id>_SHARES_CAPITAL`, `financial_operations` claim, `SACCOS_SHARE_PURCHASE` txn, holdings base-cost motion); config-driven `shareStructure` {shareValue,minShares,maxShares,autoApprove}; PENDING/APPROVED/REJECTED lifecycle with OWNER/BOARD decisions; reject refund via `creditWallet` (DR equity / CR wallet, fresh `-R` ref, txn `reversed_at`/`reversed_ref` — SUCCESS is terminal, no illegal status flip); summary totals. `scripts/test-saccos-shares.js` (38 checks, suite 46).
- ✅ **SACCOS Digital Core — Savings (increment 3)**: migration 103 member savings ledgered on the shared double-entry core (deposit `debitWallet` DR CUSTOMER_WALLET / CR per-entity LIABILITY `SACCOS<id>_SAVINGS_LIABILITY` + `SACCOS_SAVINGS_DEPOSIT` txn); config-driven `savings` {savingsType,minDeposit,maxDeposit,minBalanceToRetain,autoApproveWithdrawals}; auto-approved withdrawals release via `creditWallet` (DR liability / CR wallet); PENDING gated withdrawals reserve funds (SUM pending) and move nothing until OWNER/BOARD approve — rejections are free, SUCCESS terminal preserved; `SAV-<saccosId>-<memberNo>` accounts, member RBAC 403 + cross-entity isolation 404, platform ADMIN oversight, audit trail. `scripts/test-saccos-savings.js` (38 checks, suite 47).
- ✅ **SACCOS Digital Core — Credit (increment 4)**: migration 104 loan applications SCL-* → OWNER/BOARD approval → disbursement on the shared double-entry core (claim + `postJournal` DR per-entity ASSET `SACCOS<id>_LOANS_RECEIVABLE` / CR CUSTOMER_WALLET + `SACCOS_LOAN_DISBURSEMENT` txn); flat interest `total_repayable = principal * (1 + rate%*months/12)` with proportional principal/interest split on repayment (DR CUSTOMER_WALLET / CR LOANS_RECEIVABLE principal + CR `SACCOS<id>_INTEREST_INCOME` REVENUE) idempotent on REP-* refs + `WALLET_INSUFFICIENT_FUNDS` guard; closes at zero outstanding (CLOSED + REPAID); config `lending` {interestRate,minAmount,maxAmount,maxTermMonths,maxActiveLoans,autoDisburse} incl. manual disburse/reject paths; member RBAC 403 + cross-entity 404 + distinct ASSET/REVENUE codes + ADMIN oversight + audit. `scripts/test-saccos-credit.js` (49 checks, suite 48). Next per SACCOS scope: governance → accounting → investments.
- ✅ **SACCOS Digital Core — Governance (increment 5)**: migration 105 entity-scoped collective decisioning — `saccos_resolutions` RES-* (DRAFT→OPEN→PASSED/REJECTED/CANCELLED) + `saccos_resolution_votes` (one vote FOR/AGAINST/ABSTAIN per member per resolution via UNIQUE); OWNER/BOARD propose/open/close/cancel, ACTIVE members vote while OPEN; no money → no journal (every transition + vote audit-logged); closing tally `quorum = cast/ACTIVE ≥ quorumPercent%` and `decision = FOR/cast ≥ decisionThresholdPercent%` → PASSED else REJECTED; config `governance` {allowMemberVoting,quorumPercent,decisionThresholdPercent,votingDays} incl. officers-only voting mode; member RBAC 403 + cross-entity 404 + ADMIN oversight reads/summary + audit. `scripts/test-saccos-governance.js` (28 checks, suite 49). Next per SACCOS scope: accounting → investments.
- ✅ **SACCOS Digital Core — Accounting (increment 6)**: migration 106 entity-scoped bookkeeping + financial statements on the shared ledger — OWNER/BOARD open accounting period `PER-*` (one OPEN at a time, `SACCOS_ACC_PERIOD_ALREADY_OPEN`), book internal journals `ACC-*` through it via claim + `postJournal` against `SACCOS<id>_*` ledger accounts (EXPENSE: DR GENERAL_EXPENSE / CR OPERATING_CASH; INCOME: DR OPERATING_CASH / CR OTHER_INCOME), idempotent on reference (retry dedup), close into immutable CLOSED with statements snapshot (reopen CLOSED→OPEN the only escape; booking into closed period → `SACCOS_ACC_PERIOD_OPEN`); statements computed from `ledger_accounts` + `journal_entries` scoped to the entity — chart (grouped ASSET/EXPENSE/REVENUE), trial balance (`balanced=true`, DR=CR), income statement (revenue/expense/net), balance sheet (assets = liabilities + equity incl. net income); members read + OWNER/BOARD write + cross-entity 404 (S2 books never touch S1 balances) + ADMIN oversight + audit. `scripts/test-saccos-accounting.js` (36 checks, suite 50). Next per SACCOS scope: investments.
- ✅ **SACCOS Digital Core — Investments (increment 7, FINAL)**: migration 107 entity-scoped member term investments — OWNER/BOARD define products `INVP-*` {minAmount, maxAmount, annualRatePercent, termMonths, ACTIVE/ARCHIVED}; members subscribe `INV-*` auto-approved or OWNER/BOARD-approved (config.investments.autoApprove) — subscription `debitWallet` DR CUSTOMER_WALLET / CR per-entity LIABILITY `SACCOS<id>_INVESTMENTS_LIABILITY` + `SACCOS_INVESTMENT_SUBSCRIPTION` txn; redemption at maturity (flat interest = principal * rate% * term/12) via 3-leg journal DR LIABILITY principal + DR `SACCOS<id>_INVESTMENT_INTEREST_EXPENSE` (EXPENSE) / CR CUSTOMER_WALLET idempotent on `RED-*` with wallet FOR UPDATE (maturity guard `SACCOS_INV_NOT_MATURED`, CLOSED → `SACCOS_INV_STATE`, member-of-other redeem 403); reject path REJECTED moves nothing; min/max guards; cross-entity 404 + ADMIN oversight + audit. `scripts/test-saccos-investments.js` (32 checks, suite 51). **SACCOS Digital Core scope complete: foundation → shares → savings → credit → governance → accounting → investments.**
- ✅ **SACCOS Digital Core — Dividends (increment 8, EXTENSION)**: migration 108 closes the economic loop — surplus distribution from the authoritative closed accounting period. OWNER/BOARD declare a dividend run `DIV-*` against a CLOSED `saccos_accounting_period` (one run per period → `SACCOS_DIV_PERIOD_USED`, OPEN → `SACCOS_DIV_PERIOD_CLOSED`), per_share derived from `totalAmount / eligibleShares` or given directly; eligible = ACTIVE members' holdings (share_count, `SACCOS_DIV_NO_SHARES`); declaration snapshots per-holder payouts `DIVP-*` (share_count × per_share, PENDING); distribution pays each via `claimOperation` + `postJournal` DR per-entity EXPENSE `SACCOS<id>_DIVIDEND_DISTRIBUTED` / CR CUSTOMER_WALLET with wallet FOR UPDATE + `auditBalance`, idempotent per `DIVP-*` (re-distribute pays nothing extra); run → DISTRIBUTED only when no PENDING payouts remain (partial retry idempotent while DECLARED); members read own payouts, lists/summaries on ADMIN oversight, member RBAC 403, cross-entity 404, audit trail. `scripts/test-saccos-dividends.js` (23 checks, suite 52). Base sequence now extended: foundation → shares → savings → credit → governance → accounting → investments → dividends.
- ✅ **SACCOS Digital Core — Overdue Arrears & Late-Fee Accrual (increment 13)**: migration 113 adds `days_late`/`late_fee` to installments and completes the OVERDUE lifecycle (v112 introduced the status but nothing set it now fixed). **Accrual**: PENDING installments whose due date passes the grace window (`saccos.config.lending.graceDays` default 0) are flipped OVERDUE - on any read (`loans/:loanId/installments`) or by board `POST /:id/loans/recompute-arrears` (audit `SACCOS_ARREARS_RECOMPUTE`) - with `days_late` and late fee `total * lateFeePercent% * ceil(days_late/30)` (default 2% per 30 started) computed deterministically and idempotently per day. **Payment**: paying an OVERDUE installment debits `total + late_fee` from the borrower wallet (gated `WALLET_INSUFFICIENT_FUNDS`) journaled DR wallet / CR `SACCOS<id>_LOANS_RECEIVABLE` + CR `SACCOS<id>_INTEREST_INCOME` + CR `SACCOS<id>_LATE_FEE_INCOME` (REVENUE, auto-upserted), while `amount_outstanding` decrements only by the installment total (the fee is income); `SACCOS_LOAN_INSTALLMENT_PAYMENT` txn records the fee in meta. **Arrears view**: governing `GET /:id/loans/arrears` returns a fresh snapshot (overdue count, arrears total, late fees, total installments); member list shows overdue count/fees per loan. Suite 57 (27 checks) green twice + regressions 54/55/56 - caught + fixed a read-before-write bug in the pay path (`fetchInstallment` was reading outside the txn) and a harness `jsonb_set` nested-create limitation (merge-`||`). Sequence: foundation → shares → savings → credit → governance → accounting → investments → dividends → funds/reserves → member exit & settlement → statements/reports → loan installments & interest accrual → overdue arrears & late-fee accrual.
- ✅ **SACCOS Digital Core — Loan Installments & Interest Accrual (increment 12)**: migration 112 adds `saccos_loan_installments` (UNIQUE (loan_id, installment_no), REPI-* refs, PENDING/PAID/OVERDUE) - a per-loan amortisation schedule layered over the credit module. **Schedule**: term_months rows splitting principal_part + interest_part exactly (sums == principal and == total_repayable − principal; last row absorbs rounding); generated deterministically and idempotently - materialised lazily on first read (already-disbursed loans get it automatically) or explicitly by OWNER/BOARD `POST /:id/loans/:loanId/installments/generate` (regenerate → SACCOS_LOAN_INSTALLMENTS_EXIST). **Payment** (member pays own loan; OWNER/BOARD may pay any): REPI-* claim + 3-leg journal DR CUSTOMER_WALLET / CR `SACCOS<id>_LOANS_RECEIVABLE` (principal_part) + CR `SACCOS<id>_INTEREST_INCOME` (interest_part; REVENUE account auto-upserted) with wallet FOR UPDATE; `amount_outstanding -= total`; loan CLOSED + application REPAID at zero; `SACCOS_LOAN_INSTALLMENT_PAYMENT` txn + `saccos_loan_repayments` row. Strict ordering - later installment → SACCOS_LOAN_INSTALLMENT_ORDER; duplicate → SACCOS_LOAN_INSTALLMENT_ALREADY_PAID. Governing summary `GET /:id/loans/installments/summary` (member 403). `scripts/test-saccos-installments.js` (32 checks, suite 56) - suites 54/55 regression green; ledger evidence: interest recognised as 900 revenue (CR−DR), receivable back to 0. Sequence: foundation → shares → savings → credit → governance → accounting → investments → dividends → funds/reserves → member exit & settlement → statements/reports → loan installments & interest accrual.
- ✅ **SACCOS Digital Core — Member Statements & Regulatory Reports (increment 11)**: migration 111 adds `saccos_regulatory_reports` (UNIQUE (saccos_id, as_of)) for a dated, immutable regulatory picture. **Statements**: consolidated per-member position - savings account + shares holdings + loans (outstanding) + investments + dividend payouts + exit record + wallet - self-readable (`statements/mine`) including EXITED members (settlement proof), OWNER/BOARD read any member (`members/:memberId/statement`, sibling reads blocked SACCOS_RBAC), non-members 404. **Regulatory** (`reports/regulatory`, OWNER/BOARD/ADMIN): aggregates computed live from the shared double-entry ledger - per-entity `SACCOS<id>_*` codes totaled by account type (LIABILITY/EQUITY/REVENUE hold credit, ASSET/EXPENSE debit) for savings/investment/fund liabilities, dividend expense, revenue/expense/net-income - plus member counts, share totals, ACTIVE loans outstanding, loan-loss reserves and exit settlements from the saccos tables, then UPSERTed as a snapshot (re-run refreshes same row). **Management** (`reports/management?months=N`): monthly gross flows - savings deposits/withdrawals, loan disbursements/repayments, dividends paid, fund contributions, exit settlements. Audit `SACCOS_REGULATORY_REPORT`/`SACCOS_MANAGEMENT_REPORT`. `scripts/test-saccos-reports.js` (28 checks, suite 55) - regression suite 54 also green. This increment caught + fixed a real increment-10 bug: exit settlement paid the whole entity savings liability to a leaver; now settles the member's own deposit book value. Sequence: foundation → shares → savings → credit → governance → accounting → investments → dividends → funds/reserves → member exit & settlement → statements/reports.
- ✅ **SACCOS Digital Core — Member Exit & Settlement (increment 10)**: migration 110 adds `saccos_member_exits` (SXC-*) and closes the membership lifecycle. ACTIVE member (never OWNER; OWNER exits stay open) settles book value atomically via `settleAndExit`: savings `creditWallet` DR per-entity LIABILITY `SACCOS<id>_SAVINGS_LIABILITY` / CR wallet (`SXC-*-:SAV`); shares redeemed at holdings book value (claim `SXC-*-:SHR` + balanced postJournal DR EQUITY `SACCOS<id>_SHARES_CAPITAL` / CR wallet + wallet FOR UPDATE + audit + holdings zeroed); every PENDING dividend payout on DECLARED runs paid on the payout's own `DIVP-*` reference (DR EXPENSE `SACCOS<id>_DIVIDEND_DISTRIBUTED` / CR wallet, marked PAID) so a later board distribution settles only remaining members without double-paying leavers. One settlement per member (UNIQUE member_id, `SACCOS_EXIT_ALREADY_SETTLED`); membership → EXITED; re-settle → `SACCOS_MEMBER_STATUS_INVALID`. Self-only, RBAC 403 on OWNER settle, cross-entity 404, `SACCOS_EXIT_SETTLED` audit. `scripts/test-saccos-exits.js` (23 checks, suite 54). Sequence: foundation → shares → savings → credit → governance → accounting → investments → dividends → funds/reserves → member exit & settlement → statements/reports.
- ✅ **SACCOS Digital Core — Fund Management + Loan-Loss Reserves (increment 9, EXTENSION)**: migration 109 adds two ledger-backed economic features. **Funds**: OWNER/BOARD carve per-SACCOS LIABILITY buckets `SACCOS<id>_FUND_<CODE>` (FND-*, code-regex + UNIQUE(saccos_id,code) + target/minimum balances, ledger account auto-upserted); ACTIVE members contribute (`debitWallet` DR CUSTOMER_WALLET / CR fund account, `SACCOS_FUND_CONTRIBUTION` txn + fund meta, FNC-*, WALLET_INSUFFICIENT_FUNDS pre-check); OWNER/BOARD transfer between funds (balanced journal DR from / CR to, FTF-*, SACCOS_FUND_SAME/INSUFFICIENT guards); archive only at zero residual (SACCOS_FUND_HAS_BALANCE); members read own contributions (mine), ADMIN oversight summaries, cross-entity 404. **Loan-loss**: boards provision a disbursed loan ONCE (UNIQUE(saccos_id,loan_id) → SACCOS_LLR_EXISTS) — DR `SACCOS<id>_LOAN_LOSS_EXPENSE` (EXPENSE) / CR `SACCOS<id>_LOAN_LOSS_RESERVES` (LIABILITY), LLR-*; release reverses the journal (SACCOS_LLR_STATE on re-release); provisions surface live in the ledger-computed income statement. `scripts/test-saccos-funds.js` (47 checks, suite 53). Sequence: foundation → shares → savings → credit → governance → accounting → investments → dividends → funds/reserves → member exit & settlement → statements/reports.
...
- ✅ **Payment requests (request-to-pay)** (migration 098: `payment_requests`). User asks a contact
  ...
  Dashboard surfaced: Banking.jsx "Payment Requests" tab — create request-to-pay (amount/note/
  expiry), pay incoming, cancel outgoing, lifecycle badges, sw/en i18n.
...
- ✅ **Merchant invoices** (migration 099: `merchant_invoices`): merchants issue itemised
  ...
  **`scripts/test-merchant-invoices.js` — 38 checks (INV-* lifecycle, MERCH-* settlement,
  idempotent re-pay, guards, RBAC) wired into CI.** Dashboard surfaced: Merchant.jsx
  "Merchant Invoices" card — issue itemised invoice, list with status/expiry, pay-by-code,
  lookup preview, cancel, copyable payment link `/pay/:code`, sw/en i18n.
...
- ✅ **P2 institutional documents drafted** (`docs/institutional/`): AFK-INST-02 BRD, 03 PRD,
  04 SRS (FR/NFR), 06 System Architecture, 07 Data Architecture, 08 API Specification (standardises
  return-shape contracts C4), 09 Database Design, 10 Security Architecture, 11 Threat Model (STRIDE),
  17 Reconciliation Specification, 21 Test Strategy (accurate CI suite list), 22 Release Management,
  23 Operations Runbook — registry + gap-analysis statuses updated.

- ✅ **Canonical 27-document set DRAFT-complete** (`docs/institutional/`): P3 added AFK-INST-01
  ...
  next is formal approval cycling.
...
- ✅ **Recurrence Automation Scheduler** (migration 067: `recurrence_rules`, `recurrence_executions`). `recurrenceService.js` provides a background interval-driven scheduler (started from `server.js`, gated by `DISABLE_CRON`, tunable via `RECURRENCE_INTERVAL_MS`) that auto-dispatches due recurring tasks to their services: `AUTO_SAVINGS` (sweep wallet→savings pool with ledger postings), `CONTRIBUTION_CYCLE` (auto-create next VICOBA contribution cycle), `PAYROLL_RUN` (auto-generate+approve payroll for an active schedule), **`STANDING_INSTRUCTION` (recurring wallet transfer: SI-* idempotent journal transfer + transactions + wallet_ledger + SMS + audit, payload `{fromUserId, toPhoneNumber, amount, note?, minAmount?}`)**. Every execution is recorded to `recurrence_executions`. Endpoints: `POST/GET /api/recurrence/rules`, `PATCH /api/recurrence/rules/:id`, `GET /api/recurrence/executions`, `POST /api/recurrence/sweep`. New admin Recurrence page at `/dashboard/recurrence` (task-type select + payload hint now includes `STANDING_INSTRUCTION` with SI-* payload shape). **`scripts/test-recurrence.js` — 43 checks (SI-* transfer contract, balanced journal, idempotent re-sweep, insufficient-balance FAILED, RBAC, disabled-skip) wired into CI.**
...
- ✅ **MULTI-COUNTRY DEPLOYMENT & REGULATORY LICENSING** (migration 089): `supported_countries` gains compliance attributes — `calling_code` (MSISDN→country resolution), `max_daily_transfer_limit`, `withholding_tax_rate`, `kyc_doc_type_required`, `regulatory_license_status` (LICENSED/SANDBOX/PENDING with CHECK), `regulatory_license_name`, `local_support_phone`, `local_compliance_email`; seeded per corridor (TZ=LICENSED/Bank of Tanzania, KYC NIDA, 10% WHT, 20M/day; KE/UG/RW/BI/ZM/NG/GH per regulator). `user_daily_transfer_totals` (PK user,country,UTC date) enforces daily caps; `GOVERNMENT_WHT` (LIABILITY) + `REMITTANCE_CLEARING` (ASSET) ledger accounts added. `countryService.js` — `getCountryByPhone` (longest calling-code match), `getCountryForUser` (country_code flag → phone fallback), `getRegulatoryConfig`, `computeWithholdingTax`, `enforceDailyTransferLimit`/`recordDailyTransfer` (transaction-scoped, 402 `REGULATORY_DAILY_LIMIT`), and `executeTransfer` now applies source-country withholding tax (DR wallet → CR `GOVERNMENT_WHT`) and enforces/records the daily cap atomically. New API: `GET /api/countries` (active countries + calling codes + license snapshot) and `GET /api/countries/me` (my country config + `todayUsage`/`remaining`) — auth-gated; admin tunes limits via existing `PUT /api/admin/countries/:id` (whitelist widened). This pass also fixed a latent bug the suite exposed: `executeTransfer` targeted `REMITTANCE_CLEARING` which was never seeded — now idempotently seeded. `scripts/test-multi-country.js` — 31 checks (calling-code/license listings, per-country regulatory config via `/me`, fee+principal+withholding wallet math, `GOVERNMENT_WHT` journal postings, daily-cap 402 enforcement, admin limit tuning, cumulative usage reporting, RBAC) wired into CI after the USSD suite.**TCRA USSD SHORTCODE REGISTRY** (migration 097): `supported_countries` += `ussd_shortcode`/`ussd_shortcode_status` (PENDING|APPROVED with CHECK, default PENDING); TZ seeded `*150*87`; admin flips the filing status via `PUT /api/admin/countries/:id` (COUNTRY_UPDATED audit); surfaced as an `ussd` block in `/api/countries` + `/api/countries/me`; shortcodes stored without the `#` dial terminator (guard-friendly; MNO appends at dial time). `scripts/test-tcra-ussd.js` — 22 checks (schema evidence, CHECK rejects invalid status, TZ baseline, snapshot + `/me` surfacing, admin APPROVED lifecycle + audit, 401 RBAC) wired into CI — closes the last AFK-INST-13 ACTION REQUIRED row (the offshore TCRA filing itself remains a Compliance action tracked in the registry).
- ✅ **FIELD PARTNER API (KIVA-STYLE)** (migration 091): `field_partners` += `user_id` (unique partial index), `phone_number`, `operator_name`, `available_balance`, `updated_at`; new `field_partner_loans` (PENDING/DISBURSED/REPAID/DEFAULTED/CANCELLED, computed `total_due`, unique `loan_reference` FPL-*) and `field_partner_repayments` (unique `reference`). `src/services/fieldPartnerService.js` — `listPartners/getPartner/getPartnerForUser/createPartner/updatePartner/bindUser` (binds a staff member, promotes MJUMBE→FIELD_PARTNER)/`fundPartner` (DR `SUSPENSE` → CR `PARTNER_BALANCE`, pool +=)/`createLoan` (`total_due = amount * (1 + rate/100)`)/`disburseLoan` (DR `PARTNER_BALANCE` → CR `CUSTOMER_WALLET`, pool check FOR UPDATE)/`recordRepayment`+`selfRepay` (DR `CUSTOMER_WALLET` → CR `PARTNER_BALANCE`, REPAID when paid ≥ total_due)/`listLoans/getBorrowerLoans/getSummary`. API at `/api/field-partners` (catalog, admin CRUD + fund/bind, partner-context loans, borrower self-serve; permit-scoped RBAC yielding 403 `FIELD_PARTNER_UNAUTHORIZED`). Ledger-invariant: pool projection `available_balance` always mirrors `PARTNER_BALANCE` net. `scripts/test-field-partners.js` — 29 checks wired into CI after the ledger-integrity suite. **UI parity**: dashboard `FieldPartners.jsx` at `/dashboard/field-partners` (sw/en i18n; admin register/bind/fund + partner book with disburse/repay, partner console, borrower self-repay) and Flutter `field_partners_screen.dart` (drawer "Mashirika ya Uga"; my-loans self-repay + field console with summary/book/create-loan).
- ✅ **DEVICE SECURITY & ANTI-FRAUD BINDING** (migration 092): `users.device_policy` (PERMISSIVE | TRUSTED_ONLY, CHECK-constrained, default PERMISSIVE) + `trusted_devices.is_trusted` (+ composite index). `deviceService` now prefers a client `x-device-fingerprint` (hex 32–128) and falls back to a server-derived sha256(ip|user-agent|accept-language); new helpers `trustedDeviceStatus`/`touchDevice`/`noteNewDevice` (first-seen device → MEDIUM `DEVICE` `fraud_alerts` row)/`setDevicePolicy`. Middleware `enforceDeviceTrust` (mounted on `POST /api/wallet/transfer` + `/withdraw`) blocks TRUSTED_ONLY users with an unknown/untrusted/unregistered fingerprint at 403 `DEVICE_NOT_TRUSTED` while PERMISSIVE users keep moving money (alerts only). `createDeviceRateLimiter` — in-memory sliding-window per-fingerprint limiter (bypassed by `RATE_LIMIT_DISABLED` unless `DEVICE_RATE_LIMIT_ALWAYS=true`) mounted on `/wallet`, 429 `DEVICE_RATE_LIMIT_EXCEEDED`; default 300 req / 15 min (`DEVICE_RATE_MAX`/`DEVICE_RATE_WINDOW_MS`). Self-service API at `/api/devices` (`GET` list, `POST` register current device, `PUT /policy`, `PUT /:id/trust`, `DELETE /:id`); `authRequired` now loads `device_policy` so guards need no extra query. `scripts/test-device-binding.js` — 19 checks (PERMISSIVE baseline, TRUSTED_ONLY block → register unlock, revoke/re-trust/delete re-block, policy validation + RBAC, direct limiter unit: 3 pass / 4th 429) wired into CI after the field-partners suite. **UI parity**: dashboard `Devices.jsx` at `/dashboard/devices` (sw/en i18n; PERMISSIVE/TRUSTED_ONLY toggle, register current device, revoke/restore/remove) and Flutter `devices_screen.dart` (+ `core/device_fingerprint.dart` stable per-install `x-device-fingerprint` header; drawer "Vifaa na Usalama"; ApiClient gained optional headers + DELETE).
