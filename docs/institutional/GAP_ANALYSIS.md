...
| 21 | Test Strategy (AFK-INST-21) | YES — DRAFT | 0.5 | Yes | No | Levels, CI gates, suite list (accurate to CI, 34 suites incl. test-recurrence), flake policy | Approve baseline | QA Lead | 2 — MEDIUM ✅ delivered |
...
STRIDE), 17 (reconciliation), 21 (test strategy; test-procurement + test-tcra-ussd + test-restore-verify + test-recurrence → 34 suites), 22 (release mgmt),
...
migration-067 placeholder; suite count → 34). ADR backlog continued 2026-09-08
