...
| 21 | Test Strategy (AFK-INST-21) | YES — DRAFT | 0.8 | Yes | No | Levels, CI gates, suite list (accurate to CI, 37 suites incl. test-recurrence + test-payment-requests + test-merchant-invoices + test-marketplace), flake policy | Approve baseline | QA Lead | 2 — MEDIUM ✅ delivered |
...
STRIDE), 17 (reconciliation), 21 (test strategy; test-procurement + test-tcra-ussd + test-restore-verify + test-recurrence + test-payment-requests + test-merchant-invoices + test-marketplace → 37 suites), 22 (release mgmt),
