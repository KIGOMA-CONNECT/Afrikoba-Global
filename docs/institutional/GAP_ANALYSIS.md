...
| 21 | Test Strategy (AFK-INST-21) | YES — DRAFT | 1.2 | Yes | No | Levels, CI gates, suite list (accurate to CI, 41 suites incl. test-recurrence + test-payment-requests + test-merchant-invoices + test-marketplace + test-family-guardian + test-projection-cache + test-support + test-currency), flake policy, k6 + uptime workflows | Approve baseline | QA Lead | 2 — MEDIUM ✅ delivered |
...
STRIDE), 17 (reconciliation), 21 (test strategy; test-procurement + test-tcra-ussd + test-restore-verify + test-recurrence + test-payment-requests + test-merchant-invoices + test-marketplace + test-family-guardian + test-projection-cache + test-support → 41 suites), 22 (release mgmt),
