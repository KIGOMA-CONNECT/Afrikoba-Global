...
// Phase 47: PFE suite master index & reconciliation certificate (expert-only) + export + pdf
router.get('/projects/ops/pfe-certificate', async (req, res, next) => {
  try {
    return res.json(await projectFinance.getPfeMasterCertificate({ userId: req.user.id, role: req.user.role }));
  } catch (e) { next(e); }
});

router.get('/projects/ops/pfe-certificate/export', async (req, res, next) => {
  try {
    const csv = await projectFinance.exportPfeMasterCertificateCsv({ userId: req.user.id, role: req.user.role });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=pfe-certificate.csv');
    return res.send(csv);
  } catch (e) { next(e); }
});

router.get('/projects/ops/pfe-certificate/pdf', async (req, res, next) => {
  try {
    const data = await projectFinance.getPfeMasterCertificate({ userId: req.user.id, role: req.user.role });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=pfe-certificate.pdf');
    projectFinance.renderPfeMasterCertificatePdf(data, res);
  } catch (e) { next(e); }
});
...
router.get('/projects/ops/schedule-monitor/pdf', async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=platform-schedule-monitor.pdf');
    projectFinance.renderScheduleMonitorPdf(data, res);
  } catch (e) { next(e); }
});

// Phase 53: platform KPI & health dashboard (JSON / CSV / PDF) - expert only
router.get('/projects/ops/kpi', async (req, res, next) => {
  try {
    return res.json(await projectFinance.getPlatformKpiDashboard({ userId: req.user.id, role: req.user.role }));
  } catch (e) { next(e); }
});

router.get('/projects/ops/kpi/export', async (req, res, next) => {
  try {
    const csv = await projectFinance.exportPlatformKpiCsv({ userId: req.user.id, role: req.user.role });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=platform-kpi.csv');
    return res.send(csv);
  } catch (e) { next(e); }
});

router.get('/projects/ops/kpi/pdf', async (req, res, next) => {
  try {
    const data = await projectFinance.preparePlatformKpiPdf({ userId: req.user.id, role: req.user.role });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=platform-kpi.pdf');
    projectFinance.renderPlatformKpiPdf(data, res);
  } catch (e) { next(e); }
});
...
router.get('/projects/ops/settlement-closeout', async (req, res, next) => {
  try {
    return res.json(await projectFinance.getSettlementCloseOutRegister({ role: req.user.role }));
  } catch (e) { next(e); }
});

router.get('/projects/ops/settlement-closeout/export', async (req, res, next) => {
  try {
    const csv = await projectFinance.exportSettlementCloseOutRegisterCsv({ role: req.user.role });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=settlement-closeout.csv');
    return res.send(csv);
  } catch (e) { next(e); }
});

router.get('/projects/ops/settlement-closeout/pdf', async (req, res, next) => {
  try {
    const data = await projectFinance.getSettlementCloseOutRegister({ role: req.user.role });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=settlement-closeout.pdf');
    projectFinance.renderSettlementCloseOutRegisterPdf(data, res);
  } catch (e) { next(e); }
});
...
router.get('/projects/ops/kyc-register', async (req, res, next) => {
  try {
    return res.json(await projectFinance.getKycComplianceRegister({ role: req.user.role }));
  } catch (e) { next(e); }
});

router.get('/projects/ops/kyc-register/export', async (req, res, next) => {
  try {
    const csv = await projectFinance.exportKycComplianceRegisterCsv({ role: req.user.role });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=kyc-register.csv');
    return res.send(csv);
  } catch (e) { next(e); }
});

router.get('/projects/ops/kyc-register/pdf', async (req, res, next) => {
  try {
    const data = await projectFinance.getKycComplianceRegister({ role: req.user.role });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=kyc-register.pdf');
    projectFinance.renderKycComplianceRegisterPdf(data, res);
  } catch (e) { next(e); }
});
...
router.get('/projects/ops/tax-register', async (req, res, next) => {
  try {
    return res.json(await projectFinance.getTaxRegister({ role: req.user.role }));
  } catch (e) { next(e); }
});

router.get('/projects/ops/tax-register/export', async (req, res, next) => {
  try {
    const csv = await projectFinance.exportTaxRegisterCsv({ role: req.user.role });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=tax-register.csv');
    return res.send(csv);
  } catch (e) { next(e); }
});

router.get('/projects/ops/tax-register/pdf', async (req, res, next) => {
  try {
    const data = await projectFinance.getTaxRegister({ role: req.user.role });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename=tax-register.pdf');
    projectFinance.renderTaxRegisterPdf(data, res);
  } catch (e) { next(e); }
});
